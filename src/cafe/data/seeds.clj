...
(def product-fixtures '({:name "Cafe Latte"
                         :sku "cl-123"
...
(def order-fixture
  {:user_id 1
   ...
   :line-items '({:product_id 1 :quantity 3 :price 2.99}
                 {:product_id 2 :quantity 1 :price 4.99})})

(def roles-fixtures '({:name "admin"}
                      {:name "user"}))

(def user-fixture {:name "Juan Antonio"
                   :lastname "Plaza"
                   :email "jplazaarguello@gmail.com"
                   :password "12345678"})

(def address-fixture {:firstname "Juan Antonio"
                      :lastname "Plaza"
...
(def countries-fixtures '({:name "Ecuador" :iso "EC" :iso3 "ECU" :numcode 283}
                          {:name "United States" :iso "US" :iso3 "USA" :numcode 700}))

(def provinces-fixtures '({:name "Guayas" :iso "G" :country_id 1}
...
(def status-fixtures '({:name "Pending" :machine_readable "pending"}
...
(defn seed-db []
  ...
  (products/add-product (first product-fixtures))
  (products/add-product (last product-fixtures))

...
(defmigration create-table-categories
  (up [] (create (table :categories
                        ...
                        (varchar :uri 100 :not-null)
                        (varchar :name 100 :not-null)
                        (varchar :description 255 :not-null)
                        (integer :parent_id [:refer :categories :id]))))
...
(defmigration create-table-images
  (up [] (create (table :images
                        (integer :id :primary-key :auto-inc)
                        (varchar :name 100)
                        (varchar :description 250)
                        (varchar :url 250)
                        (integer :product_id [:refer :products :id]))))
  (down [] (drop (table :images))))

(defmigration create-table-products-categories
  (up [] (create
          (table :products_categories
                 (integer :category_id :not-null [:refer :categories :id])
                 (integer :product_id :not-null [:refer :products :id])
                 (primary-key [:category_id :product_id]))))
  (down [] (drop (table :products_categories))))
